...
public class Instagram implements AuthenticatedInsta {
	...
	public Instagram(OkHttpClient httpClient, Mapper mapper, DelayHandler delayHandler) {
	...
	protected Request withCsrfToken(Request request) {
		List<Cookie> cookies = httpClient.cookieJar().loadForRequest(request.url());
...
//		return request;
	...
	public void loadAccountInfo() {
		if (this.account == null) {
			try {
				this.account = getAccountByUsername(username);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	public void login(String username, String password) throws IOException {
		...
		Request request = new Request.Builder().url(Endpoint.LOGIN_URL)
				.header(Endpoint.REFERER, Endpoint.BASE_URL + "/").post(formBody).build();
	...
	public Account getAccountByUsername(String username) throws IOException {
		...
		Response response = executeHttpRequest(request);
		try (InputStream jsonStream = response.body().byteStream()) {
			Account account = mapper.mapAccount(jsonStream);
			return account;
		}
	}
